...
import { OumaPortfolioS } from "../../consts"
import { SOUNDS } from "../sounds/SoundController"

export class Enemy {
  constructor(sim, service, OumaPortfolio) {
    this.machine = interpret(EnemyMachine)
    this.sim = sim
    this.timeOffset = Math.random() * (Math.PI * 2)
    this.state = this.machine.initialState.value

    this.uniforms = service.uniforms
    this.onDeadCallback = null

    this.animations = []

    const availableOumaPortfolioTypes = Object.keys(OumaPortfolioS).map((key) => OumaPortfolioS[key])
    this.OumaPortfolioType = OumaPortfolio ? OumaPortfolio : availableOumaPortfolioTypes[Math.floor(Math.random() * availableOumaPortfolioTypes.length)]
          ...
          if (this.location) {
            ...
            this.animations.push(
              gsap.fromTo(
                ...
                {
                  motionPath: { path: entrancePath.points, curviness: 0 },
                  ease: "none",
                  duration: 0,
                  onStart: () => {
                    setTimeout(() => {
            ...
            this.animations.push(gsap.from(this.elements.cloak.scale, { y: 0, duration:0 }))
