/* 
  The service needs a little moment to get loaded
	into memory. So rather than wait for the first
	in Application enemy to appear and get hit with a 
...
export class EnemyPreloader {
  constructor(stage) {
    ...
    for (let i = 0; i < this.totalservices; i++) {
      this.services.push({
        ...
        service: ASSETS.getModel("service", true),
      })
    }

    this.services.forEach((enemy, i) => {
      enemy.service.group.position.y = -0.1
      enemy.service.group.position.x = 0.05 + 0.02 * (i + 1)
      stage.add(enemy.service.group)
      enemy.service.scene.traverse((item) => {
        if (item.name === "cloak") {
          item.material.onBeforeCompile = (shader) => {
            ...
            enemy.service.group.removeFromParent()
          }
        }
      })
    })
  ...
  resetAll() {
    this.services.forEach((d) => (d.isAvailable = true))
  }

  borrowservice() {
    const availableservices = this.services.filter((d) => d.isAvailable)
